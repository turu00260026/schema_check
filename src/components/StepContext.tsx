  ...
  const buttonStyle = (opt: string): React.CSSProperties => {
    if (!confirmed) {
      return {
        ...
        background: selected === opt ? '#fff8e1' : '#ffffff',
        color: '#222',
        cursor: 'pointer',
        minWidth: 140,
        transition: 'all 0.2s',
    ...
    if (opt === step.correct) {
      return {
        padding: '16px 32px', fontSize: 26, fontWeight: 'bold', borderRadius: 16,
        border: '3px solid #2e7d32', background: '#66bb6a', color: '#fff',
        cursor: 'default', minWidth: 140, transition: 'all 0.2s', boxShadow: 'none',
      };
    }
    if (opt === selected) {
      return {
        padding: '16px 32px', fontSize: 26, fontWeight: 'bold', borderRadius: 16,
        border: '3px solid #b71c1c', background: '#ef5350', color: '#fff',
        cursor: 'default', minWidth: 140, transition: 'all 0.2s', boxShadow: 'none',
      };
    }
    return {
      padding: '16px 32px', fontSize: 26, fontWeight: 'bold', borderRadius: 16,
      border: '3px solid #bbb', background: '#f5f5f5', color: '#aaa',
      cursor: 'default', minWidth: 140, transition: 'all 0.2s', boxShadow: 'none',
    };
  };
